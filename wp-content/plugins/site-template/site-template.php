...
function st_get_url($action, $tid)
{
	...
	return add_query_arg($actions,((get_major_version() == 3)?"./ms-admin.php":"./wpmu-admin.php"));
}
...
/*************************************************************** 
* Adds the "Site Template" menu to the Super Admin menu panel
* @param $blog_id for the current row
***************************************************************/
add_action('admin_menu', 'st_admin_menu');
function st_admin_menu() 
{
	add_submenu_page(((get_major_version() == 3)?'ms-admin.php':'wpmu-admin.php'),'Site Template Options', 'Site Template', 8, __FILE__, 'st_options');
}
...
function st_options()
{
	...
	<?php if ($action != 'children') { ?>
		...
		<form name="update-template-form" action="<?php echo add_query_arg($actions,((get_major_version() == 3)?"./ms-admin.php":"./wpmu-admin.php")); ?>" method="post">
		<table class="widefat"> 
			<thead> 
				<tr> 
