...
class bpModActions extends bpModeration {
	...
	function  __construct() {
		...
		add_action('init',array(&$this,'route_action'));
...
', 'bp-moderation'), $cont->item_url, $flag_count, bp_core_get_user_domain($cont->item_author) );
	...
	function check_flag($type,$id,$id2,$reporter){
		...
		$sql = $wpdb->prepare("SELECT c.content_id, f.flag_id FROM {$this->contents_table} c LEFT OUTER JOIN {$this->flags_table} f
				ON (c.content_id = f.content_id AND f.reporter_id = %d ) WHERE c.item_type = %s AND c.item_id = %d AND c.item_id2 = %d LIMIT 1",
				$reporter, $type, $id, $id2);
	...
	function count_flags($cont_id){
		...
		$sql = "SELECT COUNT(*) FROM {$this->flags_table} f WHERE content_id =";
