...
class CreationWizard {
	...
	function wpmu_new_blog ( $new_blog_id ) {
		...
		if ( is_super_admin() && isset($_POST['new_blog_owner']) && trim( $_POST['new_blog_owner'] ) != '' ) {
			...
			if ( $userid != null ) {
				add_user_to_blog( $new_blog_id, $userid, 'administrator' );
				/*
			...
			*****************************************************************/
			...
			foreach ($type_option_model_blog_options as $option) {
				if( !in_array($option->option_name, $untouchable_options) ) {
					...
					add_option($option->option_name, $new_option_value);
					//update_option($option->option_name, $new_option_value);
	...
	function signup_form() {
		...
		?>
		<label>Select a template for the new site:</label>
		...
		foreach( $type_options_array as $key => $type_option ) {
			...
			if ( $type_option['adminonly'] != 'yes' || current_user_can( 'manage_network_options' ) ) {
				...
				$checked = ( $first/*$_POST['type_option_model_blog'] == $type_option['modelblog']*/ ) ? ' checked="checked" ':''; $first = false;
		...
		?>/><strong>No Template</strong></label>
		...
		if ($features_options_array[0]->name) {
	...
	function scw_signup() {
		...
		$url = get_site_url(1) . '/wp-signup.php';
		//echo "<p>$url</p>";
		//header('Location: ' . $url);
